...
# This fuction has all the merchantConfig properties getters and setters methods
  class Merchantconfig
    def initialize(cybsPropertyObj)
      ...
      @logSize = cybsPropertyObj['logSize']&.to_i
    ...
    def validateMerchantDetails()
      ...
      @log_obj = if @enableLog.to_s.squish.downcase == 'true'
        @logSize ||= Constants::DEFAULT_LOG_SIZE
        @logFilename = Constants::DEFAULT_LOGFILE_NAME if @logFilename.to_s.empty?
        @logDirectory = Constants::DEFAULT_LOG_DIRECTORY
        Dir.mkdir(Constants::DEFAULT_LOG_DIRECTORY) unless Dir.exist?(@logDirectory)
        @log_obj = Log.new(@logDirectory, @logFilename, @logSize, @enableLog)
        @log_obj.logger.info('START> =======================================')
        ApiException.new.apiwarning(Constants::WARNING_PREFIX + Constants::INVALID_LOG_DIRECTORY + File.expand_path(@logDirectory), @log_obj)
        @log_obj
      else
        nil
      end
    ...
    def logAllProperties(propertyObj)
      ...
      @log_obj.logger.info('MERCHCFG >' + propertyObj.to_s) if @log_obj
    end
